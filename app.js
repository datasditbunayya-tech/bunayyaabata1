...
const DEFAULT_USERS = {
  ...
  wali: { username: 'wali', password: 'wali123', role: 'wali', name: 'Wali Siswa' }
};
...
function loadInitialData() {
  ...
  if (SCRIPT_URL) {
    ...
    getAllData().then(data => {
      ...
      studentData = allData.filter(d => d.type === 'master_siswa');
      populateTeacherDropdowns();
      console.log('Data refreshed from sheet for login');
    }).catch(e => console.log('Offline mode or init load failed'));
  }
}
...
function handleLogin() {
  ...
  if (role === 'wali') {
    ...
    if (studentData.length === 0) loadFallbackData();
    ...
    if (student && password === username) {
      ...
      showToast(`Selamat datang, Wali Ananda ${student.nama}!`, 'success');
      return;
    } else {
       // Check default wali login as backup
       ...
       if (username === defaultWali.username && password === defaultWali.password) {
         currentUser = {
          ...
          document.getElementById('loading-overlay').style.display = 'flex';
          initializeApp();
          showToast(`Selamat datang, Wali Siswa!`, 'success');
          return;
       }
    }
  }
    ...
    if (currentUser.studentNis) {
      setTimeout(() => {
        ...
        if (classSelect && studentSelect) {
          ...
          // Disable filters for specific wali to prevent viewing other students
          classSelect.disabled = true;
          studentSelect.disabled = true;
          
          showToast(`Data Ananda ${currentUser.studentName} dimuat`, 'success');
        }
...
function loadFallbackData() {
  studentData = [
    ...
    { nis: "203", nama: "Muhammad Arkaan", class_name: "Kelas 5" },
    { nis: "205", nama: "AHMAD ZAEN MALEEQ", class_name: "Kelas 4" },
    { nis: "206", nama: "ANWAR SHOFWAN", class_name: "Kelas 4" }
  ];
...
// Update applyRoleAccess to show/hide logo button
...
applyRoleAccess = function() {
  ...
  if (btnUploadLogo) {
    if (currentUser && currentUser.role === 'admin') {
      btnUploadLogo.classList.remove('hidden');
    } else {
      btnUploadLogo.classList.add('hidden');
    }
  }
